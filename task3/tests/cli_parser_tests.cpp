...
TEST( CliParserTests, ReturnsErrorWhenNoArgumentsAreProvided ) {
	...
	const auto result = CliParser::parse( args );

	EXPECT_EQ( result.mode, CliParser::CliMode::Error );
...
TEST( CliParserTests, ReturnsHelpModeWhenHelpOptionIsProvided ) {
	...
	EXPECT_EQ( result.mode, CliParser::CliMode::ShowHelp );
}
...
TEST( CliParserTests, ParsesLogFileAndFilters ) {
	...
	ASSERT_EQ( result.mode, CliParser::CliMode::Execute );

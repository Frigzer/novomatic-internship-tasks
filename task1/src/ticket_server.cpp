...
TicketServer::TicketServer(std::vector<Ticket> tickets,
...
std::variant<PurchaseSuccess, PurchaseFailure> TicketServer::finalizePurchase(
    ...
    // Sprawdzamy czy da się wydać resztę (symulacja na kopii kasboxu + wpłacone monety)
    ...
    // Skoro wszystko ok, finalizujemy stan fizyczny
...
void TicketServer::cleanupExpiredReservations() {
    ...
    // Nowoczesny sposób usuwania elementów spełniających warunek (C++20 std::erase_if)

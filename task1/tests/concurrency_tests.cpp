...
TEST(ConcurrencyTests, OnlyOneClientCanReserveLastTicket) {
    ...
    TicketServer server(
        {Ticket{1, 350, "normal", TicketStatus::Available, std::nullopt}},
        CoinInventory({{200, 1}, {100, 5}, {50, 2}}),
...
TEST(ConcurrencyTests, NumberOfSuccessfulReservationsMatchesTicketPool) {
    ...
    for (int i = 1; i <= 5; ++i) {
        tickets.push_back(Ticket{static_cast<TicketId>(i), 350, "normal", TicketStatus::Available, std::nullopt});
    }

    TicketServer server(
        std::move(tickets),
        CoinInventory({{200, 2}, {100, 10}, {50, 10}}),
        std::chrono::seconds(60),
        [&now] { return now; });
...
TEST(ConcurrencyTests, ConcurrentPurchasesDoNotOversellTickets) {
    ...
    TicketServer server(
        std::move(tickets),
        CoinInventory({{200, 3}, {100, 10}, {50, 10}}),
        std::chrono::seconds(60),
        [&now] { return now; });
    ...
    for (int i = 0; i < thread_count; ++i) {
        threads.emplace_back([&server, &success_count, &failure_count, i]() {
            ...
            auto result = server.finalizePurchase(
